...
function getTrimReg(c) {
    return new RegExp('^[' + c + ']+|[' + c + ']+$', 'gi');
}

function getClassReg(c) {
    return new RegExp('(^|\\s)(' + c + ')($|\\s)', 'i');
}
...
function toggleHide(e, d) {
    e.style.display = (e.style.display != (d ? d : d = '') ? d : 'none');
}
...
function orz(n, d) {
    return (isNaN(d) ? parseInt(n || 0) : parseFloat(n || d)) || 0;
}
...
function dataToBlob(data) {
    if (URL_API && URL_API.createObjectURL) {
        ...
        var data = Uint8Array.from(TOS.map.call(data, v => v.charCodeAt(0))),
            size = data.length,
            url = URL_API.createObjectURL(new Blob([data], {
                'type': type
            }));
        if (url) {
            return {
...
function saveDL(data, fileName, param) {
    ...
    if (addTime == 'after') addTime = 1;
    else
    if (addTime == 'before') addTime = -1;
    else addTime = orz(addTime);

    var data = (
        typeof data === 'object' ?
        JSON.stringify(
            data, param.jsonReplacerFunc || null, '\t'
        ) :
        '' + data
    );
...
async function draw() {
    ...
    function drawRandomSprite(
        x0, y0, pixel_cols, pixel_rows, pixel_size, colors_count, colors_random_max, ver_symmetry, hor_symmetry, drawFrameFileNamePefix
    ) {

        function drawPixelRect(x, y) {
            ctx.fillRect(
                pixel_size * x, pixel_size * y, pixel_size, pixel_size
            );
        }
            ...
            for (var x = 0; x < random_cols; x++) {
                ...
                if (r < colors.length) {
                    ...
                    if (drawFrameFileNamePefix) {
                        saveDL(
                            canvas.toDataURL(), [drawFrameFileNamePefix, 'y' + y, 'x' + x].join('_'), {
                                ext: 'png',
                                tryAsBlob: false
                            }
                        );
    ...
    var canvas = cre('canvas'),
        ...
        fileName = (
            [
                getFormattedTime(0, 1), 'canvas', [param.canvas_width, param.canvas_height].join('x'), 'with', [param.sprites_per_row, param.sprites_per_col].join('x'), 'sprites', [param.sprite_width, param.sprite_height].join('x'), param.pixel_size + 'px', param.colors_per_sprite + '_colors'
            ].join('_')
        );
    ...
    for (var row_i = 0; row_i < param.sprites_per_row; row_i++)
        for (var col_i = 0; col_i < param.sprites_per_col; col_i++) {
            ...
            drawRandomSprite(
                x0, y0, param.sprite_width, param.sprite_height, param.pixel_size, param.colors_per_sprite, colors_random_max, checkbox.ver_symmetry, true, (
                    save_draw_frames ?
